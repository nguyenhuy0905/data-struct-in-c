...
/// @brief header node
Node *head = NULL;
/// @brief a pointer to the head node. Useful in certain situations, like adding
/// a node to the head
Node *pHead;
...
Node *add_node(char data[64], int position) {
  ...
  in the main function, then every time input_data is changed,
  every-single-value in the list gets changed also.
  */
...
/// @brief A list of input options corresponding to the print_options() function
/// down here
...
/// @brief A list of action to be done after getting the position. Either add a
/// node at that position, or remove one
enum position_action { REMOVE = 0, ADD = 1 };

/// @brief Give a prompt corresponding to action to be taken and record the
/// user-input position
void get_position(int *position, int action) {
  while (*position < 0 || *position > list_length) {
    printf("Please input the position to %s: ",
           (action == 1) ? "add this node" : "remove a node");
    scanf(" %d", position);
